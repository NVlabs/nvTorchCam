...
class TestCameras(unittest.TestCase):
    def setUp(self):
        self.pts = torch.tensor([[[-1.0841, -1.1539,  0.5728],
        ...
        self.pix = torch.tensor([[[ 1.0993,  0.9824],
         [-0.5771,  0.6910],
         [-1.7918, -0.1632],
         [-0.4430,  0.4839]],
        ...
        self.pos_depths = torch.tensor([[0.5890, 0.2369, 0.8817, 0.8845],
        [0.6597, 0.5008, 0.7557, 0.8208]])

        self.neg_depths = torch.tensor([[-1.8059, -0.0886,  1.0758,  0.1906],
        [-0.8546,  0.2547,  1.0518,  0.0417]])
    ...
    def project_to_pixel_test(self, camera, pts, unit_vec, rtol=None, atol=None):
        ...
        assert torch.all( valid_unproj[valid_proj] ) #assert valid_proj implies valid_unproj
        torch.testing.assert_close(pts[valid_proj,:], recon_pts[valid_proj,:], rtol=rtol, atol=atol)
    ...
    def test_pinhole_camera(self):
        ...
        z_min = torch.ones(2)*(-10) #even for negative z_min pinhole camera unproject is injective
    ...
    def test_equirectangular_camera(self):
        ...
        camera = cameras.EquirectangularCamera.make(phi_range, theta_range, min_distance)
    ...
    def test_opencv_fisheye(self):
        ...
        camera = cameras.OpenCVFisheyeCamera.make(flat_intrinsics, distortion_coeffs, theta_max, distance_min)
    ...
    def test_opencv_fisheye_compared_to_opencv(self):
        ...
        torch_cam = cameras.OpenCVFisheyeCamera.make(flat_intrinsics, distortion_coeffs, max_theta, torch.tensor(0).float())
        ...
        pix_cv, _ = cv2.fisheye.projectPoints(pts.unsqueeze(1).numpy(),torch.zeros(3).numpy(), torch.zeros(3).numpy(), intrinsics.numpy(), distortion_coeffs.numpy())
    ...
    def test_forward_backward_fisheye(self):
        unit_vec = False
        b=2
        fwpoly = torch.tensor([0.0000, 412.9887, -54.4291, 111.8926, -32.7999]).unsqueeze(0).expand(b,-1)
        bwpoly = torch.tensor([0.0000e+00,  2.4903e-03, -4.3055e-08, -1.3850e-09,  9.8613e-13]).unsqueeze(0).expand(b,-1)
        ...
        theta_max = torch.ones(b)*(3/4)*np.pi
        distance_min = torch.ones(b)*(0.0)
        camera = cameras.BackwardForwardPolynomialFisheyeCamera.make(flat_intrinsics, fwpoly,  bwpoly, theta_max, distance_min)
    ...
    def test_forward_backward_fisheye_concatenate_different_degrees(self):
        ...
        bwpoly_4 = torch.tensor([0.0000e+00,  2.4903e-03, -4.3055e-08, -1.3850e-09,  9.8613e-13]).unsqueeze(0)
        fwpoly_5 = torch.tensor([0.0000, 412.9887, -54.4291, 111.8926, -32.7999, 1.2]).unsqueeze(0)
        bwpoly_6 = torch.tensor([0.0000e+00,  2.4903e-03, -4.3055e-08, -1.3850e-09,  9.8613e-13, 1.3]).unsqueeze(0)
        ...
        camera_a = cameras.BackwardForwardPolynomialFisheyeCamera.make(flat_intrinsics, fwpoly_4,  bwpoly_4, theta_max, distance_min)
        camera_b = cameras.BackwardForwardPolynomialFisheyeCamera.make(flat_intrinsics, fwpoly_5,  bwpoly_6, theta_max, distance_min)
    ...
    def test_kitti360_fisheye(self):
        ...
        camera = cameras.Kitti360FisheyeCamera.make(flat_intrinsics,k1,k2,xi,theta_max, distance_min)
    ...
    def test_heterogeneous_batch_get_rays_and_project(self):
        ...
        pin_camera = cameras.PinholeCamera.make(intrinsics= torch.tensor([[1,1,0.4,0.5],[2,2.5,-0.4,0.2] ]))
        ortho_camera = cameras.OrthographicCamera.make(intrinsics=torch.tensor([[1,1.2,-0.2,0.1]]))
        ...
        origin_hetero, dirs_hetero, valid_hetero = hetero_camera.pixel_to_ray(torch.cat((proj_pts_pin, proj_pts_ortho)), False)
    ...
    def test_heterogeneous_batch_tensor_like_functions(self):
        ...
        pin_camera = cameras.PinholeCamera.make(intrinsics=pin_values[...,:4], z_min=pin_values[...,4])
        ortho_values = torch.rand(3,2,5)
        ortho_camera = cameras.OrthographicCamera.make(intrinsics=ortho_values[...,:4], z_min=ortho_values[...,4])
        ...
        actual = heterogeneous_ortho_pin_to_tensor( torch.cat((pin_camera, ortho_camera),dim=1).reshape(-1) )
        torch.testing.assert_close(expected, actual)

        expected = torch.cat((pin_values, ortho_values),dim=1).reshape(9,2,5)
        actual = heterogeneous_ortho_pin_to_tensor( torch.cat((pin_camera, ortho_camera),dim=1).reshape(9,2) )
        torch.testing.assert_close(expected, actual)

        expected = torch.cat((pin_values, ortho_values),dim=1).reshape(-1,2,5)
        actual = heterogeneous_ortho_pin_to_tensor( torch.cat((pin_camera, ortho_camera),dim=1).reshape(-1,2) )
        torch.testing.assert_close(expected, actual)

        #unsqueeze
        expected = torch.cat((pin_values, ortho_values),dim=1).unsqueeze(1)
        actual = heterogeneous_ortho_pin_to_tensor( torch.cat((pin_camera, ortho_camera),dim=1).unsqueeze(1) )
        torch.testing.assert_close(expected, actual)
        ...
        actual = heterogeneous_ortho_pin_to_tensor( torch.cat((pin_camera, ortho_camera),dim=1).unsqueeze(1).expand(-1,4,-1) )
        torch.testing.assert_close(expected, actual)
       
        #squeeze
        expected = torch.cat((pin_values, ortho_values),dim=1).unsqueeze(1).squeeze(2)
        actual = heterogeneous_ortho_pin_to_tensor( torch.cat((pin_camera, ortho_camera),dim=1).unsqueeze(1).squeeze(2) )
        torch.testing.assert_close(expected, actual)

        expected = torch.cat((pin_values, ortho_values),dim=1).unsqueeze(1).squeeze(1)
        actual = heterogeneous_ortho_pin_to_tensor( torch.cat((pin_camera, ortho_camera),dim=1).unsqueeze(1).squeeze(1) )
        torch.testing.assert_close(expected, actual)

        expected = torch.cat((pin_values, ortho_values),dim=1).unsqueeze(1).squeeze()
        actual = heterogeneous_ortho_pin_to_tensor( torch.cat((pin_camera, ortho_camera),dim=1).unsqueeze(1).squeeze() )
        torch.testing.assert_close(expected, actual)
        ...
        actual = heterogeneous_ortho_pin_to_tensor(torch.stack((pin_ortho_camera, pin_pin_camera),dim=1))
        torch.testing.assert_close(expected, actual)
    ...
    def test_crop(self):
        ...
        z_min = torch.tensor(1e-6) #even for negative z_min pinhole camera unproject is injective
        ...
        camera_fish = cameras.OpenCVFisheyeCamera.make(flat_intrinsics, distortion_coeffs, theta_max, distance_min)

        for camera in [camera_pin, camera_ortho, camera_fish]:
            origin, dir, valid = camera.get_camera_rays((4,6), False)
            cropped_camera = camera.crop(torch.tensor([1,4,1,3]), normalized=False, image_shape=dir.shape[-3:-1])
            cropped_origin, cropped_dir, cropped_valid = cropped_camera.get_camera_rays((2,3), False)
    ...
    def test_apply_affine(self):
        ...
        mult_left_intrinsics = utils.intrinsics_matrix_from_flat_intrinsics(camera_mult_left._values['affine'])
        ...
        mult_right_intrinsics = utils.intrinsics_matrix_from_flat_intrinsics(camera_mult_right._values['affine'])
    ...
    def test_is_central(self):
        ...
        distortion_coeffs = torch.tensor([0.1,0.2,0.1,0.1])
        flat_intrinsics = torch.tensor([2,2,-0.4,0.5])
        opencv_fisheye = cameras.OpenCVFisheyeCamera.make(flat_intrinsics, distortion_coeffs, max_theta, torch.tensor(0).float())
