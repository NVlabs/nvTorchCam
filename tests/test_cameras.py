...
class TestCameras(unittest.TestCase):
    ...
    def pixel_to_ray_test(self, camera, pix, depth, unit_vec, rtol=None, atol=None):
        ...
        recon_pix, recon_depth, valid_proj = camera.project_to_pixel(
            pts, unit_vec)
        valid = valid_proj & valid_unproj
        torch.testing.assert_close(
            pix[valid, :], recon_pix[valid, :], rtol=rtol, atol=atol)
        torch.testing.assert_close(
            depth[valid], recon_depth[valid], rtol=rtol, atol=atol)
    ...
    def test_pinhole_camera(self):
        ...
        camera = cameras.PinholeCamera.make(
            intrinsics=flat_intrinsics, z_min=z_min)
    ...
    def test_orthographic_camera(self):
        ...
        # even for negative z_min pinhole camera unproject is injective
        z_min = torch.ones(2)*(-10)
        camera = cameras.OrthographicCamera.make(
            intrinsics=flat_intrinsics, z_min=z_min)
    ...
    def test_opencv_fisheye_compared_to_opencv(self):
        ...
        intrinsics = utils.intrinsics_matrix_from_flat_intrinsics(
            flat_intrinsics)
        pix_cv, _ = cv2.fisheye.projectPoints(pts.unsqueeze(1).numpy(), torch.zeros(
            3).numpy(), torch.zeros(3).numpy(), intrinsics.numpy(), distortion_coeffs.numpy())
    ...
    def test_opencv(self):
        ...
        ks = torch.tensor([[0.1, 0.05, 0.01, 0.02, 0.01, 0.005],
                           [0.08, 0.07, 0.01, 0.03, 0.02, 0.003]])
    ...
    def test_opencv_compared_to_opencv(self):
        ...
        pix_cv, _ = cv2.projectPoints(pts.unsqueeze(1).numpy(), torch.zeros(
            3).numpy(), torch.zeros(3).numpy(), intrinsics.numpy(), opencv_params)
        pix_cv = torch.from_numpy(pix_cv).squeeze(1)
        torch.testing.assert_close(pix_cv[valid, :], pix[valid, :])
    ...
    def test_forward_backward_fisheye(self):
        ...
        b = 2
        fwpoly = torch.tensor(
            [0.0000, 412.9887, -54.4291, 111.8926, -32.7999]).unsqueeze(0).expand(b, -1)
        bwpoly = torch.tensor([0.0000e+00,  2.4903e-03, -4.3055e-08, -
                              1.3850e-09,  9.8613e-13]).unsqueeze(0).expand(b, -1)
        cx_cy = torch.tensor([969.006165, 599.42144]
        ...
        unit_vec = True
        self.project_to_pixel_test(
            camera, self.pts, unit_vec, atol=0.01, rtol=0.1)
        self.pixel_to_ray_test(camera, self.pix, self.pos_depths, unit_vec)
    ...
    def test_forward_backward_fisheye_concatenate_different_degrees(self):
        ...
        fwpoly_4 = torch.tensor(
            [0.0000, 412.9887, -54.4291, 111.8926, -32.7999]).unsqueeze(0)
        bwpoly_4 = torch.tensor(
            [0.0000e+00,  2.4903e-03, -4.3055e-08, -1.3850e-09,  9.8613e-13]).unsqueeze(0)
        fwpoly_5 = torch.tensor(
            [0.0000, 412.9887, -54.4291, 111.8926, -32.7999, 1.2]).unsqueeze(0)
        bwpoly_6 = torch.tensor(
            [0.0000e+00,  2.4903e-03, -4.3055e-08, -1.3850e-09,  9.8613e-13, 1.3]).unsqueeze(0)
    ...
    def test_heterogeneous_batch_get_rays_and_project(self):
        ...
        proj_pts_pin, depth_pts_pin, valid_pin = pin_camera.project_to_pixel(
            pts_pin)
        proj_pts_ortho, depth_pts_ortho, valid_ortho = ortho_camera.project_to_pixel(
            pts_ortho)
        proj_pts_hetero, depth_pts_hetero, valid_hetero = hetero_camera.project_to_pixel(
            pts_hetero)
        ...
        origin_pin, dirs_pin, valid_pin = pin_camera.pixel_to_ray(
            proj_pts_pin, False)
        origin_ortho, dirs_ortho, valid_ortho = ortho_camera.pixel_to_ray(
            proj_pts_ortho, False)
    ...
    def test_heterogeneous_batch_tensor_like_functions(self):
        ...
        hetero_camera = torch.cat(
            (pin_camera, ortho_camera, ortho_camera), dim=1)
        actual = heterogeneous_ortho_pin_to_tensor(hetero_camera)
        torch.testing.assert_close(expected, actual)
        ...
        hetero_camera = torch.cat(
            (pin_camera, ortho_camera), dim=1).transpose(0, 1)
        actual = heterogeneous_ortho_pin_to_tensor(hetero_camera)
        torch.testing.assert_close(expected, actual)
        ...
        expected = torch.cat((pin_values, ortho_values),
                             dim=1).reshape(9, 2, 5)
        actual = heterogeneous_ortho_pin_to_tensor(
        ...
        # expand
        expected = torch.cat((pin_values, ortho_values),
                             dim=1).unsqueeze(1).expand(-1, 4, -1, -1)
        actual = heterogeneous_ortho_pin_to_tensor(
        ...
        values = torch.cat((pin_values, ortho_values),
                           dim=1).transpose(0, 1)  # (6,3,5)
        hetero_cam = torch.cat((pin_camera, ortho_camera),
                               dim=1).transpose(0, 1)  # (6,3)
        torch.testing.assert_close(
            values[:, 0:2], heterogeneous_ortho_pin_to_tensor(hetero_cam[:, 0:2]))
    ...
    def test_heterogeneous_batch_devolves_to_homogeneous(self):
        ...
        for k in devolved_pin_camera._values.keys():
            torch.testing.assert_close(
                devolved_pin_camera._values[k], pin_camera._values[k])
    ...
    def test_crop(self):
        ...
        camera_pin = cameras.PinholeCamera.make(
            intrinsics=flat_intrinsics, z_min=z_min)
        camera_ortho = cameras.OrthographicCamera.make(
            intrinsics=flat_intrinsics, z_min=z_min)
        ...
        for camera in [camera_pin, camera_ortho, camera_fish]:
            ...
            cropped_camera = camera.crop(torch.tensor(
                [1, 4, 1, 3]), normalized=False, image_shape=dir.shape[-3:-1])
    ...
    def test_apply_affine(self):
        ...
        torch.testing.assert_close(
            mult_left_intrinsics, torch.bmm(A, intrinsics))

        camera_mult_right = camera_pin.affine_transform(
            flat_A, multiply_on_right=True)
        mult_right_intrinsics = utils.intrinsics_matrix_from_flat_intrinsics(
            camera_mult_right._values['affine'])
        torch.testing.assert_close(
            mult_right_intrinsics, torch.bmm(intrinsics, A))
