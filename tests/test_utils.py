...
class TestCoords(unittest.TestCase):
    ...
    def test_mat_multiply(self):
        ...
        Ax = utils.mat_multiply(A,x)
        Ax_torch = torch.bmm(A.reshape(2*5,3,3), x.reshape(2*5,10*12,3).transpose(1,2)).transpose(1,2).reshape(2,5,10,12,3)
        torch.testing.assert_close(Ax, Ax_torch)
    ...
    def test_compute_jacobian(self):
        ...
        #(b, n, dim) -> (b, n, dim)
        f = lambda x: torch.einsum('bij,bnj->bni', A, x)
