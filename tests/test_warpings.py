...
class TestRenderSphereImage(unittest.TestCase):
    ...
    def test_ray_sphere_intersection(self):
        ...
        intersection, distance, valid = warpings.ray_sphere_intersection(
            origin, dirs)
        ...
        intersection_b, distance_b, valid_b = warpings.ray_sphere_intersection(
            origin, dirs)

        torch.testing.assert_close(intersection.unsqueeze(
            0).unsqueeze(0).expand(2, 3, -1, -1), intersection_b)
        torch.testing.assert_close(valid.unsqueeze(
            0).unsqueeze(0).expand(2, 3, -1), valid_b)
        torch.testing.assert_close(distance.unsqueeze(
            0).unsqueeze(0).expand(2, 3, -1), distance_b)
    ...
    def test_render_sphere_image(self):
        ...
        intrinsics[0, 0] = 6
        intrinsics[1, 1] = 6
        cam_fish = cameras.Kitti360FisheyeCamera.make(
            intrinsics, k1, k2, xi, theta_max, distance_min)

        to_world = torch.eye(4)
        to_world[2, 3] = -4
        res = (8, 8)
        image_fish, distance_fish, _ = warpings.render_sphere_image(
            cam_fish, to_world, res, 1)
        ...
        expected_distance = torch.tensor([[[nan,    nan,    nan,    nan,    nan,    nan,    nan,    nan],
                                           ...
                                           [nan,    nan,    nan, 3.3692,
                                               3.3692,    nan,    nan,    nan],
                                           [nan,    nan, 3.3692, 3.0563,
                                               3.0563, 3.3692,    nan,    nan],
                                           [nan,    nan, 3.3692, 3.0563,
                                               3.0563, 3.3692,    nan,    nan],
                                           [nan,    nan,    nan, 3.3692,
                                               3.3692,    nan,    nan,    nan],
        ...
        torch.testing.assert_close(torch.nan_to_num(
            image_fish), torch.nan_to_num(expected_image), atol=1e-4, rtol=1e-3)
        torch.testing.assert_close(torch.nan_to_num(distance_fish), torch.nan_to_num(
            expected_distance), atol=1e-4, rtol=1e-3)

        # test with batching
        cam_fish = cam_fish.unsqueeze(0).unsqueeze(0).expand(2, 3)
        to_world = to_world.unsqueeze(0).unsqueeze(0).expand(2, 3, -1, -1)
        image_fish_b, distance_fish_b, _ = warpings.render_sphere_image(
            cam_fish, to_world, res, 1)

        torch.testing.assert_close(torch.nan_to_num(image_fish_b), torch.nan_to_num(
            expected_image.unsqueeze(0).unsqueeze(0).expand(2, 3, -1, -1, -1)), atol=1e-4, rtol=1e-3)
        torch.testing.assert_close(torch.nan_to_num(distance_fish_b), torch.nan_to_num(
            expected_distance.unsqueeze(0).unsqueeze(0).expand(2, 3, -1, -1, -1)), atol=1e-4, rtol=1e-3)
...
class TestBackwardWarping(unittest.TestCase):
    ...
    def test_backwarp_warp_pts(self):
        ...
        trg_distance = trg_distance.squeeze(0)
        src_pts, src_pts_depth, valid_mask = warpings.backwarp_warp_pts(
            cam, trg_distance, cam, trg_to_src, depth_is_along_ray=True)
        ...
        expected_src_pts_depth = torch.tensor([[nan,    nan, 4.3750, 4.1250, 3.8750, 3.6250,    nan,    nan],
                                               [nan, 4.6250, 4.3750, 4.1250,
                                                   3.8750, 3.6250, 3.3750,    nan],
                                               [4.8750, 4.6250, 4.3750, 4.1250,
                                                   3.8750, 3.6250, 3.3750, 3.1250],
                                               [4.8750, 4.6250, 4.3750, 4.1250,
                                                   3.8750, 3.6250, 3.3750, 3.1250],
                                               [4.8750, 4.6250, 4.3750, 4.1250,
                                                   3.8750, 3.6250, 3.3750, 3.1250],
        ...
        torch.testing.assert_close(torch.nan_to_num(
            src_pts), torch.nan_to_num(expected_src_pts), atol=1e-4, rtol=1e-4)
        torch.testing.assert_close(torch.nan_to_num(
            src_pts_depth), torch.nan_to_num(expected_src_pts_depth))
        torch.testing.assert_close(valid_mask, expected_valid_mask)

        # test multiple target images
        src_pts_t2, src_pts_depth_t2, valid_mask_t2 = warpings.backwarp_warp_pts(cam, trg_distance, cam.unsqueeze(
            0).expand(2), trg_to_src.unsqueeze(0).expand(2, -1, -1), depth_is_along_ray=True)

        torch.testing.assert_close(torch.nan_to_num(src_pts.unsqueeze(
            0).expand(2, -1, -1, -1)), torch.nan_to_num(src_pts_t2))
        torch.testing.assert_close(torch.nan_to_num(src_pts_depth.unsqueeze(
            0).expand(2, -1, -1)), torch.nan_to_num(src_pts_depth_t2))
        torch.testing.assert_close(valid_mask.unsqueeze(
            0).expand(2, -1, -1), valid_mask_t2)
...
class TestResampleByIntrinsics(unittest.TestCase):
    ...
    def test_resample_by_intrinsics(self):
        ...
        to_world[2, 3] = -4
        res = (8, 8)
        image_fish, distance_fish = warpings.render_sphere_image(
            cam_fish, to_world, res, 1)
...
class TestCropImageResize(unittest.TestCase):
    ...
    def test_crop_resize_image(self):
        ...
        crop_image, valid = warpings.crop_resize_image(
            image, lrtb, normalized=False, out_shape=crop_image_expected.shape[-2:])
        torch.testing.assert_close(crop_image, crop_image_expected)

        # test list
        crop_image2, valid = warpings.crop_resize_image(
            [image, image], lrtb, normalized=False, out_shape=crop_image_expected.shape[-2:], interp_mode=['bilinear', 'nearest'])
        ...
        [origin_cropped, dirs_cropped], _ = warpings.crop_resize_image(
            [origin, dirs], lrtb, normalized=False, out_shape=crop_image_expected.shape[-2:])

        cam_cropped = cam.crop(lrtb, normalized=False,
                               image_shape=dirs.shape[-2:])
        origin_from_cropped_cam, dirs_from_cropped_cam, _ = cam_cropped.get_camera_rays(
            dirs_cropped.shape[-2:], True)
