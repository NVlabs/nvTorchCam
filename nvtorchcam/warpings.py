...
def backwarp_warp_pts(trg_cam: cameras.CameraBase,
    ...
    Returns:
        src_pts: (*batch_shape, *group_shape, *depthmaps_per_camera, h, w, pixel_dim) points to interpolate src for warping
    ...
    assert trg_depth.shape[:len(
        batch_shape)] == batch_shape, 'invalid depth shape in backward_warp_pts'
    ...
    depthmaps_per_cam_shape, _ = utils._get_group_shape(
        batch_shape, trg_depth.shape[:-2])
    group_numel = group_shape.numel()

    trg_cam = trg_cam.reshape(batch_numel)  # (b)
    src_cam = src_cam.reshape(batch_numel, group_numel)  # (b,g)
    trg_cam_to_src_cam = trg_cam_to_src_cam.reshape(
        batch_numel, group_numel, 4, 4)
    trg_depth = trg_depth.reshape(batch_numel, -1, *hw)  # (b,d,h,w)

    trg_pc, valid_unproj = trg_cam.unproject_depth(
        trg_depth, depth_is_along_ray=depth_is_along_ray)  # (b,d,h,w,3) (b,d,h,w)
    trg_pc = trg_pc.unsqueeze(
        1).expand(-1, group_numel, -1, -1, -1, -1)  # (b,g,d,h,w,3)
    src_pc = utils.apply_affine(trg_cam_to_src_cam, trg_pc)  # (b,g,d,h,w,3)

    src_pts, src_pts_depth, valid_proj = src_cam.project_to_pixel(
        src_pc, depth_is_along_ray)  # (b,g,d,h,w,pixel_dim)

    valid_mask = valid_unproj.unsqueeze(1) & valid_proj
    if src_pts.size(-1) == 2:
        valid_mask = valid_mask & utils.in_image(src_pts)

    src_pts = src_pts.reshape(
        *batch_shape, *group_shape, *depthmaps_per_cam_shape, *hw, -1)
    valid_mask = valid_mask.reshape(
        *batch_shape, *group_shape, *depthmaps_per_cam_shape, *hw)
    src_pts_depth = src_pts_depth.reshape(
        *batch_shape, *group_shape, *depthmaps_per_cam_shape, *hw)
...
def backward_warp(trg_cam: cameras.CameraBase,
    ...
    for src_image_i, mode_i, padding_mode_i in zip(src_image, interp_mode, padding_mode):
        ...
        if isinstance(src_cam, cameras.CubeCamera):
            src_image_warped_i = utils.samples_from_cubemap(
                src_image_i, src_pts, mode=mode_i)
        else:
            src_image_warped_i = utils.samples_from_image(
                src_image_i, src_pts, mode=mode_i, align_corners=align_corners, padding_mode=padding_mode_i)
        if set_invalid_pix_to_nan:
            src_image_warped_i = torch.where(valid_mask.unsqueeze(
                len(src_cam.shape)), src_image_warped_i, torch.nan)
...
def resample_by_intrinsics(src_image: TensorOrTensorList,
    ...
    trg_to_src = torch.eye(4, device=src_cam.device).reshape(
        *([1]*len(batch_shape)), 4, 4).repeat(*batch_shape, 1, 1)
    if rotation_trg_to_src is not None:
        trg_to_src[..., :3, :3] = rotation_trg_to_src

    trg_depth = torch.ones(trg_size, device=src_cam.device).reshape(
        *([1]*(len(batch_shape))), *trg_size).expand(*batch_shape, *trg_size)
    src_image_warped, _, valid_mask = backward_warp(
        trg_cam, trg_depth, src_cam, src_image, trg_to_src, interp_mode, padding_mode, depth_is_along_ray, set_invalid_pix_to_nan)
...
def get_consistancy_check_data(trg_cam: cameras.CameraBase,
    ...
    origin, dirs, valid_unproj = src_cam.pixel_to_ray(
        src_pts, depth_is_along_ray)
...
def fuse_depths_mvsnet(trg_cam: cameras.CameraBase,
    ...
    n_recon_pixel_grid, recon_trg_depth, valid_mask = get_consistancy_check_data(
        trg_cam, trg_depth, src_cam, src_depth, trg_cam_to_src_cam, depth_is_along_ray, interp_mode, padding_mode)
    recon_pixel_grid = utils.pixel_pts_from_normalized_pts(
        n_recon_pixel_grid, res)  # (*batch_shape, N, h, w, 2)

    n_pixel_grid = trg_cam.get_normalized_grid(
        res).unsqueeze(-4)  # (*batch_shape, 1, h, w, 2)
    pixel_grid = utils.pixel_pts_from_normalized_pts(
        n_pixel_grid, res)  # (*batch_shape, 1, h, w, 2)
    ...
    rel_depth_err = torch.abs(trg_depth - recon_trg_depth) / \
        trg_depth  # (*batch_size, N, h, w)
    # (*batch_size, N, h, w)
    rel_depth_err_mask = rel_depth_err < relative_depth_error_threshold

    # (*batch_size, N, h, w)
    src_masks = valid_mask & pixel_error_mask & rel_depth_err_mask
    _, _, trg_mask = trg_cam.get_camera_rays(
        res, depth_is_along_ray)  # (*batch_size, h, w)
    ...
    fused_depth = torch.sum(depths_to_fuse*fusion_masks,
                            dim=-3) / fusion_masks_sum  # (*batch_size, h, w)
    # (*batch_size, h, w)
...
def stereo_rectify(images: TensorOrTensorList,
    ...
    displacement_cam0_to_cam1 = torch.nn.functional.normalize(
        to_world[..., 1, :3, 3] - to_world[..., 0, :3, 3], dim=-1)  # (*b, 3)
    ...
    R_new_to_world = R_new_to_world.reshape(-1,
                                            3, 3).unsqueeze(1).expand(-1, 2, -1, -1)
    R_old_to_world_inv = to_world[..., :3,
                                  :3].transpose(-1, -2).reshape(-1, 2, 3, 3)
    R = torch.bmm(R_old_to_world_inv.flatten(0, 1),
                  R_new_to_world.flatten(0, 1)).reshape(*cams.shape, 3, 3)
...
def affine_transform_image(image: TensorOrTensorList,
    ...
    grid = utils.get_normalized_grid(
        out_shape, device=image[0].device)  # (h, w, 2)
    ...
    for image_i, padding_mode_i, interp_mode_i in zip(image, padding_mode, interp_mode):
        transformed_image.append(utils.samples_from_image(
...
def crop_resize_image(image: TensorOrTensorList,
    ...
    affine = utils.crop_to_affine(
        lrtb, normalized=normalized, image_shape=image_shape)
